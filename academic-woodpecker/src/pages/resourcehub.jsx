...
const ResourceHub = () => {
  ...
  const fetchFavorites = async () => {
    try {
      const [resourceRes, docRes] = await Promise.all([
        resourcesApi.getFavorites(),
        analysisApi.getFavoriteDocuments()
      ]);
      
      if (resourceRes.data && resourceRes.data.resources) {
        const favoriteIds = new Set(resourceRes.data.resources.map(r => r.id));
  ...
  const getResourceDisplay = (resource) => {
    return {
      ...
      platform: resource.platform || resource.source || '未知',
  ...
  const filteredResources = activeFilter === '全部'
    ? resources
    : resources.filter(r => r.type === activeFilter || r.类型 === activeFilter);
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredResources.map((resource) => {
                ...
                return (
                  ...
                  >
                    <div className="h-2 bg-gradient-to-r from-warning-500 via-warning-400 to-warning-500"></div>
                    <div className="p-6">
                      <div className="flex items-start justify-between mb-4">
                        ...
                        <div className="flex items-center gap-1">
                          <button
                            onClick={() => handleToggleFavorite(display.id)}
                      ...
                      <div className="flex gap-2">
                        ...
                        {display.url && (
                          <a
                            ...
                            className="flex-1 py-3 bg-gradient-to-r from-warning-500 to-warning-600 text-white font-semibold rounded-xl text-center hover:from-warning-600 hover:to-warning-700 transition-all duration-300 shadow-lg shadow-warning-500/30"
                          >
                            前往学习
                          </a>
                        )}
        ...
        <div className="fixed inset-0 bg-black/40 backdrop-blur-sm z-50 flex items-center justify-center p-4 animate-fade-in" onClick={() => setSelectedResource(null)}>
          <div className="bg-white rounded-2xl shadow-medium max-w-2xl w-full max-h-[80vh] overflow-y-auto animate-fade-in-up" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-6 space-y-6">
              <div className="p-4 bg-gradient-to-br from-warning-50 to-warning-100/50 rounded-xl border border-warning-100">
                <div className="flex items-center gap-2 mb-2">
                  <svg className="w-5 h-5 text-warning-500" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 10V3L4 14h7v7l9-11h-7z" />
                  </svg>
                  <h4 className="font-bold text-warning-800">AI 推荐理由</h4>
                </div>
                <p className="text-warning-700 leading-relaxed">{selectedResource.reason}</p>
              </div>
              ...
              {selectedResource.url && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="block w-full py-3 bg-gradient-to-r from-warning-500 to-warning-600 text-white rounded-xl font-semibold text-center hover:from-warning-600 hover:to-warning-700 transition-all duration-300 shadow-lg shadow-warning-500/30"
                >
                  前往学习
                </a>
              )}
